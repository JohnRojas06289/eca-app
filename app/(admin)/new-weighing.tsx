...
const MATERIALS: Material[] = [
  ...
  { key: 'cardboard', name: 'Cartón Corrugado', icon: 'albums-outline',         color: theme.colors.cardboard, bgColor: theme.colors.cardboardBg, pricePerKg: 350  },
...
const RECYCLERS = [
  { id: '1', name: 'Juan Pérez',      route: 'Centro Histórico' },
  { id: '2', name: 'Carlos Romero',   route: 'San Pablo Norte'  },
  { id: '3', name: 'Sofía Vargas',    route: 'El Jardín'        },
  { id: '4', name: 'Luis Martínez',   route: 'La Granja'        },
];
...
export default function AdminNewWeighingScreen() {
  ...
  const canSubmit = selectedMaterial !== null && selectedRecycler !== null && kgNum > 0;
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // await WeighingApi.create({ material: selectedMaterial, kg: kgNum, recyclerId: selectedRecycler, note });
      ...
      >
        ...
        <View style={styles.infoBanner}>
          ...
          <Text style={styles.infoBannerText}>
            El reciclador recibirá una notificación para revisar y confirmar este pesaje.
          </Text>
        </View>
        ...
        >
          {/* ── Selección de reciclador ───────────────────── */}
          <Text style={styles.fieldLabel}>Reciclador</Text>
          <View style={styles.recyclerList}>
            {RECYCLERS.map((r) => {
              const isSelected = selectedRecycler === r.id;
              return (
                <TouchableOpacity
                  key={r.id}
                  style={[styles.recyclerCard, isSelected && styles.recyclerCardSelected]}
                  onPress={() => setSelectedRecycler(r.id)}
                  activeOpacity={0.8}
                >
                  <View style={[styles.recyclerAvatar, isSelected && styles.recyclerAvatarSelected]}>
                    <Text style={[styles.recyclerInitial, isSelected && styles.recyclerInitialSelected]}>
                      {r.name[0]}
                    </Text>
                  </View>
                  <View style={styles.recyclerInfo}>
                    <Text style={[styles.recyclerName, isSelected && styles.recyclerNameSelected]}>
                      {r.name}
                    </Text>
                    <Text style={styles.recyclerRoute}>{r.route}</Text>
                  </View>
                  {isSelected && (
            ...
            {MATERIALS.map((mat) => {
              ...
              return (
                ...
                >
                  ...
                  <Text
                    style={[styles.materialName, isSelected && { color: mat.color }]}
                    numberOfLines={2}
                  >
                    {mat.name}
                  </Text>
          ...
          {material && kgNum > 0 && (
            <View style={styles.estimatedCard}>
              <View>
                <Text style={styles.estimatedLabel}>Valor a pagar al reciclador</Text>
          ...
          <TextInput
            ...
            placeholder="Ej: Material húmedo, requiere verificación..."
            placeholderTextColor={theme.colors.textMuted}
            multiline
            numberOfLines={3}
...
const styles = StyleSheet.create({
  ...
  // ── Reciclador ───────────────────────────────────────────
  recyclerList: { gap: theme.spacing.sm },
  recyclerCard: {
    flexDirection: 'row',
    alignItems: 'center',
